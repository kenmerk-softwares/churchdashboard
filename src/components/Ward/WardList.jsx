...
export default function WardList() {
  ...
  const handleNavigate = (action, value, data) => {
    navigate(Pathname.AddWard.path, { state: { action, id: value, data: data } });
  };
  ...
  useEffect(() => {
    setloading(true);
    const response = collection(db, "ward");
...
data_listener(q )
  ...
  const handleDelete = (id) => {
    deleteDoc(doc(db, "ward", id)).then(() => {
      console.log("done");
    });
  };

  const handleChapterStatusChange = (value, id, cat) => {
    updateDoc(doc(db, "ward", id), {
      status: value,
    }).then(() => {
      console.log("done");
    });
  ...
  return (
    ...
    <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
          ...
          <Card.Body className="pt-0">
            <Table hover className="user-table align-items-center">
              <thead>
                <tr>
                  ...
                  <th className="border-bottom">Ward Name</th>
                  <th className="border-bottom">Ward Number</th>
                  <th className="border-bottom">Total Houses</th>
                  <th className="border-bottom">Ward Representative</th>
                  <th className="border-bottom">Ward Representative2</th>
            ...
            <td>
              <span className="fw-normal">
             {doc.data.ward_number}
              </span>
            </td>
            <td>
              <span className="fw-normal">
             {doc.data.total_houses}
              </span>
            </td>
            <td>
               {doc.data.ward_rep_1}
