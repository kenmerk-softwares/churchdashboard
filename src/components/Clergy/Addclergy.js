...
function Addclergy() {
 ...
 const fetchorder = async () => {
   ...
   const querySnapshot = await getDocs(collection(db, 'clergy'));
...
fetchorder();
  ...
  const handleSubmit = async (event) => {
      ...
      if (location.state && location.state.action === "edit") {
           await  updateDoc(doc(db, "clergy",location.state.id), {
       ...clergyData
      ...
      <Form noValidate validated={validated} onSubmit={handleSubmit}>
        <Row className="mb-3">
          <Form.Group as={Col} md="4" controlId="validationCustom01">
            <Form.Label>Bishop/Priest</Form.Label>
            <Form.Select
              name="type"
              aria-label="Default select example"
              onChange={handleInputChange}
              value={clergyData.type}
              required
            >
