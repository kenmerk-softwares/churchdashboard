...
function Committe() {
  ...
  const handleNavigate = (action, value, data) => {
    navigate(Pathname.Addcommitte.path, { state: { action, id: value, data: data } });
  };
  ...
  useEffect(() => {
    setloading(true);
    const response = collection(db, "committee");
...
data_listener(q )
  ...
  const handleDelete = (id) => {
    deleteDoc(doc(db, "committee", id)).then(() => {
      console.log("done");
    });
  };

  const handleChapterStatusChange = (value, id, cat) => {
    updateDoc(doc(db, "committee", id), {
      status: value,
    }).then(() => {
      console.log("done");
    });
...
<div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div className="d-block mb-4 mb-md-0">
          <Breadcrumb className="d-none d-md-inline-block" listProps={{ className: "breadcrumb-dark breadcrumb-transparent" }}>
            ...
            <Breadcrumb.Item active>committe Members</Breadcrumb.Item>
          </Breadcrumb>
