...
function Prayers() {
  ...
  const handleNavigate = (action, value, data) => {
    navigate(Pathname.Addprayers.path, { state: { action, id: value, data: data } });
  };
  ...
  useEffect(() => {
    setloading(true);
    const response = collection(db, "prayers");
...
data_listener(q )
  ...
  const handleDelete = (id) => {
    deleteDoc(doc(db, "prayers", id)).then(() => {
      console.log("done");
    });
  };

  const handleChapterStatusChange = (value, id, cat) => {
    updateDoc(doc(db, "prayers", id), {
      status: value,
    }).then(() => {
      console.log("done");
    });
...
<div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div className="d-block mb-4 mb-md-0">
          <Breadcrumb className="d-none d-md-inline-block" listProps={{ className: "breadcrumb-dark breadcrumb-transparent" }}>
            ...
            <Breadcrumb.Item active>Prayers</Breadcrumb.Item>
          </Breadcrumb>
        ...
        <Table hover className="user-table align-items-center">
          <thead>
            <tr>
              ...
              <th className="border-bottom"> Prayer Title</th>
